...
async function performanceRows(where = "p.status!='HIDDEN'", args = [], client = pool) { return all(`${performanceSelect} WHERE ${where} ORDER BY p.start_at`, args, client); }
...
async function api(req,res,url){
  ...
  if(req.method==='GET'&&url.pathname==='/api/banners')return json(res,200,(await all('SELECT * FROM banners WHERE is_active=1 ORDER BY sort_order')).map(row=>({...row,link_url:normalizeLinkUrl(row.link_url)})));
  ...
  if(req.method==='GET'&&url.pathname==='/api/performances')return json(res,200,await performanceRows());
  if(req.method==='GET'&&parts[1]==='performances'&&parts[2]){const p=await get(`${performanceSelect} WHERE p.id=?`,[parts[2]]);if(!p)return json(res,404,{error:'공연을 찾을 수 없습니다.'});p.tickets=await all('SELECT * FROM ticket_types WHERE performance_id=?',[p.id]);const user=await currentUser(req);p.is_favorite=Boolean(user&&await get('SELECT 1 FROM favorites WHERE user_id=? AND performance_id=?',[user.id,p.id]));p.recommendations=await all(`${performanceSelect} WHERE p.id!=? AND p.status!='HIDDEN' ORDER BY CASE WHEN p.genre=? THEN 0 ELSE 1 END,p.start_at LIMIT 3`,[p.id,p.genre]);return json(res,200,p);}
  ...
  if(req.method==='GET'&&url.pathname==='/api/me/reservations'){const u=await requireRole(req,res);if(!u)return;return json(res,200,await all(`SELECT r.*,p.title,p.poster_url,p.start_at,p.venue_name FROM reservations r JOIN performances p ON p.id=r.performance_id WHERE r.user_id=? ORDER BY r.created_at DESC`,[u.id]));}
  if(req.method==='GET'&&parts[1]==='me'&&parts[2]==='reservations'&&parts[3]){const u=await requireRole(req,res);if(!u)return;const r=await get(`SELECT r.*,p.title,p.poster_url,p.start_at,p.venue_name,CASE WHEN r.status IN ('PAID','USED') THEN q.qr_token ELSE NULL END qr_token,q.status qr_status FROM reservations r JOIN performances p ON p.id=r.performance_id LEFT JOIN qr_tickets q ON q.reservation_id=r.id WHERE r.id=? AND (r.user_id=? OR ? IN ('MANAGER','SUPER_ADMIN'))`,[parts[3],u.id,u.role]);if(r&&r.qr_token)r.qr_verify_url=qrVerifyUrl(r.qr_token);return r?json(res,200,r):json(res,404,{error:'예매를 찾을 수 없습니다.'});}
  if(req.method==='GET'&&url.pathname==='/api/me/favorites'){const u=await requireRole(req,res);if(!u)return;return json(res,200,await all(`${performanceSelect} JOIN favorites f ON f.performance_id=p.id WHERE f.user_id=? AND p.status!='HIDDEN' ORDER BY f.created_at DESC`,[u.id]));}
