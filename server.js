...
async function initDb() {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS performance_questions (id SERIAL PRIMARY KEY, performance_id INTEGER NOT NULL REFERENCES performances(id) ON DELETE CASCADE, question_text TEXT NOT NULL, is_required INTEGER DEFAULT 1, sort_order INTEGER DEFAULT 0);
    CREATE TABLE IF NOT EXISTS performance_question_options (id SERIAL PRIMARY KEY, question_id INTEGER NOT NULL REFERENCES performance_questions(id) ON DELETE CASCADE, option_text TEXT NOT NULL, sort_order INTEGER DEFAULT 0);
...
function normalizePerformance(input) {
  ...
  const questions = (Array.isArray(input.questions) ? input.questions : []).map((question, index) => {
    ...
    const options = (Array.isArray(question.options) ? question.options : []).map(option => String(option.option_text || option).trim()).filter(Boolean);
    const uniqueOptions = [...new Set(options)];
    if (!questionText) throw new Error(`${index + 1}번째 추가 질문을 입력해 주세요.`);
    if (uniqueOptions.length < 2) throw new Error(`${index + 1}번째 추가 질문에는 선택지를 2개 이상 입력해 주세요.`);
    return { question_text: questionText, is_required: question.is_required !== false && String(question.is_required) !== '0', options: uniqueOptions };
  });
  return {
...
async function performanceQuestions(performanceId, client = pool) {
  const rows = await all(`SELECT q.id,q.question_text,q.is_required,q.sort_order,o.id option_id,o.option_text,o.sort_order option_sort_order
  ...
  for (const row of rows) {
    if (!grouped.has(row.id)) grouped.set(row.id, {id:row.id,question_text:row.question_text,is_required:Number(row.is_required)!==0,sort_order:Number(row.sort_order),options:[]});
    if (row.option_id) grouped.get(row.id).options.push({id:row.option_id,option_text:row.option_text,sort_order:Number(row.option_sort_order)});
  }
...
async function replacePerformanceQuestions(performanceId, questions, client = pool) {
  ...
  for (const [questionIndex, question] of questions.entries()) {
    const created = await get('INSERT INTO performance_questions(performance_id,question_text,is_required,sort_order) VALUES(?,?,?,?) RETURNING id', [performanceId, question.question_text, question.is_required ? 1 : 0, questionIndex + 1], client);
    for (const [optionIndex, optionText] of question.options.entries()) await run('INSERT INTO performance_question_options(question_id,option_text,sort_order) VALUES(?,?,?)', [created.id, optionText, optionIndex + 1], client);
  }
}
...
async function api(req,res,url){
  ...
  if(req.method==='POST'&&url.pathname==='/api/reservations'){
    ...
    const answers=[];
    for(const question of questions){const answer=submitted.find(item=>Number(item.questionId)===Number(question.id));const option=question.options.find(item=>Number(item.id)===Number(answer?.optionId));if(answer&&!option)return json(res,400,{error:`${question.question_text} 항목의 선택지가 올바르지 않습니다.`});if(question.is_required&&!option)return json(res,400,{error:`${question.question_text} 항목을 선택해 주세요.`});if(option)answers.push({question,option});}
