...
async function initDb() {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS performance_questions (id SERIAL PRIMARY KEY, performance_id INTEGER NOT NULL REFERENCES performances(id) ON DELETE CASCADE, question_text TEXT NOT NULL, question_type TEXT DEFAULT 'select', is_required INTEGER DEFAULT 1, sort_order INTEGER DEFAULT 0);
...
function normalizePerformance(input) {
  ...
  const questions = (Array.isArray(input.questions) ? input.questions : []).map((question, index) => {
    ...
    if (!questionType.includes('text') && uniqueOptions.length < 2) throw new Error(`${index + 1}번째 추가 질문에는 선택지를 2개 이상 입력해 주세요.`);
    return { question_text: questionText, question_type: questionType, is_required: question.is_required !== false && String(question.is_required) !== '0', options: questionType.includes('text') ? [] : uniqueOptions };
  });
...
async function performanceQuestions(performanceId, client = pool) {
  const rows = await all(`SELECT q.id,q.question_text,q.question_type,q.is_required,q.sort_order,o.id option_id,o.option_text,o.description_text,o.sort_order option_sort_order
  ...
  for (const row of rows) {
    if (!grouped.has(row.id)) grouped.set(row.id, {id:row.id,question_text:row.question_text,question_type:row.question_type || 'select',is_required:Number(row.is_required)!==0,sort_order:Number(row.sort_order),options:[]});
...
async function replacePerformanceQuestions(performanceId, questions, client = pool) {
  ...
  for (const [questionIndex, question] of questions.entries()) {
    const created = await get('INSERT INTO performance_questions(performance_id,question_text,question_type,is_required,sort_order) VALUES(?,?,?,?,?) RETURNING id', [performanceId, question.question_text, question.question_type, question.is_required ? 1 : 0, questionIndex + 1], client);
