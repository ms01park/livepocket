...
async function initDb() {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS performance_questions (id SERIAL PRIMARY KEY, performance_id INTEGER NOT NULL REFERENCES performances(id) ON DELETE CASCADE, question_text TEXT NOT NULL, question_text_en TEXT DEFAULT '', description_text TEXT DEFAULT '', description_text_en TEXT DEFAULT '', question_type TEXT DEFAULT 'select', is_required INTEGER DEFAULT 1, sort_order INTEGER DEFAULT 0);
    CREATE TABLE IF NOT EXISTS performance_question_options (id SERIAL PRIMARY KEY, question_id INTEGER NOT NULL REFERENCES performance_questions(id) ON DELETE CASCADE, option_text TEXT NOT NULL, option_text_en TEXT DEFAULT '', description_text TEXT DEFAULT '', description_text_en TEXT DEFAULT '', sort_order INTEGER DEFAULT 0);
    CREATE TABLE IF NOT EXISTS reservation_answers (id SERIAL PRIMARY KEY, reservation_id INTEGER NOT NULL REFERENCES reservations(id) ON DELETE CASCADE, question_id INTEGER REFERENCES performance_questions(id) ON DELETE SET NULL, question_text TEXT NOT NULL, answer_text TEXT NOT NULL);
    ...
    ALTER TABLE performances ADD COLUMN IF NOT EXISTS max_tickets_per_order INTEGER DEFAULT 4;
    ALTER TABLE performances ADD COLUMN IF NOT EXISTS venue_description TEXT DEFAULT '';
    ALTER TABLE performances ADD COLUMN IF NOT EXISTS title_en TEXT DEFAULT '';
    ALTER TABLE performances ADD COLUMN IF NOT EXISTS description_en TEXT DEFAULT '';
    ALTER TABLE performances ADD COLUMN IF NOT EXISTS venue_name_en TEXT DEFAULT '';
    ALTER TABLE performances ADD COLUMN IF NOT EXISTS venue_description_en TEXT DEFAULT '';
    ALTER TABLE performances ADD COLUMN IF NOT EXISTS address_en TEXT DEFAULT '';
    ALTER TABLE performances ADD COLUMN IF NOT EXISTS deposit_notice_en TEXT DEFAULT '';
    ALTER TABLE performances ADD COLUMN IF NOT EXISTS refund_policy_en TEXT DEFAULT '';
    ALTER TABLE performance_questions ADD COLUMN IF NOT EXISTS question_type TEXT DEFAULT 'select';
    ALTER TABLE performance_questions ADD COLUMN IF NOT EXISTS description_text TEXT DEFAULT '';
    ALTER TABLE performance_questions ADD COLUMN IF NOT EXISTS question_text_en TEXT DEFAULT '';
    ALTER TABLE performance_questions ADD COLUMN IF NOT EXISTS description_text_en TEXT DEFAULT '';
...
function normalizePerformance(input) {
  ...
  const questions = (Array.isArray(input.questions) ? input.questions : []).map((question, index) => {
    ...
    const questionType = String(question.question_type || 'select');
    const allowedTypes = ['select', 'short_text', 'long_text', 'checkbox_single', 'checkbox_multiple'];
    if (!allowedTypes.includes(questionType)) throw new Error(`${index + 1}번째 추가 질문의 답변 형식이 올바르지 않습니다.`);
    const options = (Array.isArray(question.options) ? question.options : []).map(option => ({
      option_text: String(option?.option_text ?? option ?? '').trim(),
      option_text_en: String(option?.option_text_en || '').trim(),
      description_text: String(option?.description_text || '').trim(),
      description_text_en: String(option?.description_text_en || '').trim(),
    })).filter(option => option.option_text);
    const uniqueOptions = options.filter((option, optionIndex) => options.findIndex(item => item.option_text === option.option_text) === optionIndex);
    if (!questionText) throw new Error(`${index + 1}번째 추가 질문을 입력해 주세요.`);
    if (!questionType.includes('text') && uniqueOptions.length < 1) throw new Error(`${index + 1}번째 추가 질문에는 선택지를 1개 이상 입력해 주세요.`);
    return { question_text: questionText, question_text_en:String(question.question_text_en || '').trim(), description_text:String(question.description_text || '').trim(), description_text_en:String(question.description_text_en || '').trim(), question_type: questionType, is_required: question.is_required !== false && String(question.is_required) !== '0', options: questionType.includes('text') ? [] : uniqueOptions };
  });
  ...
  return {
    title:String(input.title).trim(), title_en:String(input.title_en || '').trim(), genre:'', artists,
    description:String(input.description).trim(), description_en:String(input.description_en || '').trim(), poster_url:String(input.poster_url).trim(), venue_name:String(input.venue_name).trim(), venue_name_en:String(input.venue_name_en || '').trim(), venue_description:String(input.venue_description || '').trim(), venue_description_en:String(input.venue_description_en || '').trim(),
    address:String(input.address).trim(), address_en:String(input.address_en || '').trim(), start_at:String(input.start_at), booking_start_at:String(input.booking_start_at), booking_close_at:String(input.booking_close_at),
    ...
    deposit_notice_en:String(input.deposit_notice_en || '').trim(), refund_policy:String(input.refund_policy || DEFAULT_REFUND_POLICY).trim(), refund_policy_en:String(input.refund_policy_en || '').trim(), max_tickets_per_order:maxTicketsPerOrder,
    tickets: normalizedTickets, questions,
  };
}
async function performanceQuestions(performanceId, client = pool) {
  const rows = await all(`SELECT q.id,q.question_text,q.question_text_en,q.description_text question_description_text,q.description_text_en question_description_text_en,q.question_type,q.is_required,q.sort_order,o.id option_id,o.option_text,o.option_text_en,o.description_text,o.description_text_en,o.sort_order option_sort_order
    FROM performance_questions q LEFT JOIN performance_question_options o ON o.question_id=q.id
    WHERE q.performance_id=? ORDER BY q.sort_order,q.id,o.sort_order,o.id`, [performanceId], client);
  const grouped = new Map();
  for (const row of rows) {
    if (!grouped.has(row.id)) grouped.set(row.id, {id:row.id,question_text:row.question_text,question_text_en:row.question_text_en || '',description_text:row.question_description_text || '',description_text_en:row.question_description_text_en || '',question_type:row.question_type || 'select',is_required:Number(row.is_required)!==0,sort_order:Number(row.sort_order),options:[]});
    if (row.option_id) grouped.get(row.id).options.push({id:row.option_id,option_text:row.option_text,option_text_en:row.option_text_en || '',description_text:row.description_text || '',description_text_en:row.description_text_en || '',sort_order:Number(row.option_sort_order)});
  }
  return [...grouped.values()];
}
async function replacePerformanceQuestions(performanceId, questions, client = pool) {
  await run('DELETE FROM performance_questions WHERE performance_id=?', [performanceId], client);
  for (const [questionIndex, question] of questions.entries()) {
    const created = await get('INSERT INTO performance_questions(performance_id,question_text,question_text_en,description_text,description_text_en,question_type,is_required,sort_order) VALUES(?,?,?,?,?,?,?,?) RETURNING id', [performanceId, question.question_text, question.question_text_en, question.description_text, question.description_text_en, question.question_type, question.is_required ? 1 : 0, questionIndex + 1], client);
    for (const [optionIndex, option] of question.options.entries()) await run('INSERT INTO performance_question_options(question_id,option_text,option_text_en,description_text,description_text_en,sort_order) VALUES(?,?,?,?,?,?)', [created.id, option.option_text, option.option_text_en, option.description_text, option.description_text_en, optionIndex + 1], client);
  }
}
async function attachReservationAnswers(rows, client = pool) {
  const ids = rows.map(row => Number(row.id)).filter(Boolean);
  if (!ids.length) return rows;
  const placeholders = ids.map(() => '?').join(',');
  const answers = await all(`SELECT reservation_id,question_text,answer_text FROM reservation_answers WHERE reservation_id IN (${placeholders}) ORDER BY id`, ids, client);
  const grouped = new Map(ids.map(id => [id, []]));
  for (const answer of answers) grouped.get(Number(answer.reservation_id))?.push({question_text:answer.question_text,answer_text:answer.answer_text});
  return rows.map(row => ({...row, answers:grouped.get(Number(row.id)) || []}));
}
...
function publicImageUrl(value, pathPrefix, id) {
  const source = String(value || '');
  if (!/^data:/i.test(source)) return value;
  const version = crypto.createHash('sha256').update(source).digest('hex').slice(0, 12);
  return `${pathPrefix}/${id}?v=${version}`;
}
...
function posterVariant(name) {
  return ({ thumb:[120,160,78], card:[390,520,82], detail:[520,694,84] })[name] || [520,694,84];
}
async function sendPerformancePoster(res, row, variant) { const [width,height,quality]=posterVariant(variant);return sendStoredImage(res, row && {image_url:row.poster_url}, 'poster_url', width, height, quality); }
...
async function api(req,res,url){
  ...
  if(req.method==='POST'&&url.pathname==='/api/performances'){const u=await requireRole(req,res);if(!u)return;let d;try{d=normalizePerformance(await body(req));d.poster_url=await compressDataImageUrl(d.poster_url,520,694,84);}catch(e){return json(res,400,{error:e.message});}const id=await transaction(async client=>{const r=await get(`INSERT INTO performances(manager_id,title,title_en,genre,artists,description,description_en,poster_url,venue_name,venue_name_en,venue_description,venue_description_en,address,address_en,start_at,booking_start_at,booking_close_at,status,host_avatar_url,artist_avatar_url,deposit_notice,deposit_notice_en,refund_policy,refund_policy_en,max_tickets_per_order) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'OPEN',?,?,?,?,?,?,?) RETURNING id`,[u.id,d.title,d.title_en,d.genre,d.artists,d.description,d.description_en,d.poster_url,d.venue_name,d.venue_name_en,d.venue_description,d.venue_description_en,d.address,d.address_en,d.start_at,d.booking_start_at,d.booking_close_at,d.host_avatar_url,d.artist_avatar_url,d.deposit_notice,d.deposit_notice_en,d.refund_policy,d.refund_policy_en,d.max_tickets_per_order],client);for(const ticket of d.tickets)await run('INSERT INTO ticket_types(performance_id,name,price,total_quantity,remaining_quantity) VALUES(?,?,?,?,?)',[r.id,ticket.name,ticket.price,ticket.total_quantity,ticket.total_quantity],client);await replacePerformanceQuestions(r.id,d.questions,client);return r.id;});return json(res,201,{id});}
  if(req.method==='PATCH'&&parts[1]==='performances'&&parts[2]){const u=await requireRole(req,res);if(!u)return;const performance=await get('SELECT * FROM performances WHERE id=?',[parts[2]]);if(!canManagePerformance(u,performance))return json(res,403,{error:'이 공연을 수정할 권한이 없습니다.'});let d;try{d=normalizePerformance(await body(req));if(new RegExp(`^/api/performance-poster/${performance.id}(?:[?#]|$)`).test(d.poster_url))d.poster_url=performance.poster_url;else d.poster_url=await compressDataImageUrl(d.poster_url,520,694,84);}catch(e){return json(res,400,{error:e.message});}const existing=await all('SELECT * FROM ticket_types WHERE performance_id=? ORDER BY id',[performance.id]);const soldTotal=existing.reduce((sum,ticket)=>sum+Number(ticket.total_quantity-ticket.remaining_quantity),0);const nextTotal=d.tickets.reduce((sum,ticket)=>sum+Number(ticket.total_quantity),0);if(nextTotal<soldTotal)return json(res,400,{error:`이미 판매된 ${soldTotal}매보다 전체 수량을 적게 설정할 수 없습니다.`});await transaction(async client=>{await run('UPDATE performances SET title=?,title_en=?,genre=?,artists=?,description=?,description_en=?,poster_url=?,venue_name=?,venue_name_en=?,venue_description=?,venue_description_en=?,address=?,address_en=?,start_at=?,booking_start_at=?,booking_close_at=?,host_avatar_url=?,artist_avatar_url=?,deposit_notice=?,deposit_notice_en=?,refund_policy=?,refund_policy_en=?,max_tickets_per_order=? WHERE id=?',[d.title,d.title_en,d.genre,d.artists,d.description,d.description_en,d.poster_url,d.venue_name,d.venue_name_en,d.venue_description,d.venue_description_en,d.address,d.address_en,d.start_at,d.booking_start_at,d.booking_close_at,d.host_avatar_url,d.artist_avatar_url,d.deposit_notice,d.deposit_notice_en,d.refund_policy,d.refund_policy_en,d.max_tickets_per_order,performance.id],client);await run('DELETE FROM ticket_types WHERE performance_id=? AND id NOT IN (SELECT ticket_type_id FROM reservation_tickets)',[performance.id],client);const current=await all('SELECT * FROM ticket_types WHERE performance_id=? ORDER BY id',[performance.id],client);for(const [index,ticket] of d.tickets.entries()){const prev=current[index];if(prev){const sold=Number(prev.total_quantity)-Number(prev.remaining_quantity);await run('UPDATE ticket_types SET name=?,price=?,total_quantity=?,remaining_quantity=? WHERE id=?',[ticket.name,ticket.price,ticket.total_quantity,Math.max(0,ticket.total_quantity-sold),prev.id],client);}else await run('INSERT INTO ticket_types(performance_id,name,price,total_quantity,remaining_quantity) VALUES(?,?,?,?,?)',[performance.id,ticket.name,ticket.price,ticket.total_quantity,ticket.total_quantity],client);}await replacePerformanceQuestions(performance.id,d.questions,client);});return json(res,200,{ok:true});}
  ...
  if(req.method==='POST'&&url.pathname==='/api/reservations'){
    const u=await requireRole(req,res,['USER']);if(!u)return;
    const d=await body(req),reserverName=String(d.name||'').trim(),digits=String(d.phone||'').replace(/\D/g,''),phone=digits.length===11?`${digits.slice(0,3)}-${digits.slice(3,7)}-${digits.slice(7)}`:'',ticket=await get('SELECT t.*,p.booking_start_at,p.booking_close_at,p.status,p.max_tickets_per_order FROM ticket_types t JOIN performances p ON p.id=t.performance_id WHERE t.id=?',[d.ticketTypeId]),qty=Number(d.quantity);
    if(!reserverName||!phone)return json(res,400,{error:'예매자 이름과 11자리 휴대전화번호를 입력해 주세요.'});
    if(!Number.isInteger(qty)||qty<1||qty>Number(ticket?.max_tickets_per_order||4))return json(res,400,{error:`한 번에 최대 ${Number(ticket?.max_tickets_per_order||4)}매까지 예매할 수 있습니다.`});
    if(!ticket||ticket.remaining_quantity<qty)return json(res,400,{error:'선택한 티켓의 잔여 수량이 부족합니다.'});
    const questions=await performanceQuestions(ticket.performance_id),submitted=Array.isArray(d.answers)?d.answers:[];
    const answers=[];
    for(const question of questions){
      const answer=submitted.find(item=>Number(item.questionId)===Number(question.id));
      if(question.question_type==='short_text'||question.question_type==='long_text'){
        const value=String(answer?.answerText||'').trim(),maxLength=question.question_type==='short_text'?200:3000;
        if(question.is_required&&!value)return json(res,400,{error:`${question.question_text} 항목을 작성해 주세요.`});
        if(value.length>maxLength)return json(res,400,{error:`${question.question_text} 답변이 너무 깁니다.`});
        if(value)answers.push({question,answerText:value});
        continue;
      }
      const optionIds=question.question_type==='checkbox_multiple'?(Array.isArray(answer?.optionIds)?answer.optionIds:[]):[answer?.optionId].filter(Boolean);
      const uniqueIds=[...new Set(optionIds.map(Number))],selected=uniqueIds.map(id=>question.options.find(item=>Number(item.id)===id));
      if(selected.some(option=>!option))return json(res,400,{error:`${question.question_text} 항목의 선택지가 올바르지 않습니다.`});
      if(question.is_required&&!selected.length)return json(res,400,{error:`${question.question_text} 항목을 선택해 주세요.`});
      if(selected.length)answers.push({question,answerText:selected.map(option=>option.option_text).join(', ')});
    }
  ...
  if(req.method==='GET'&&parts[1]==='me'&&parts[2]==='reservations'&&parts[3]){const u=await requireRole(req,res);if(!u)return;let r=await get(`SELECT r.*,p.title,p.poster_url,p.start_at,p.venue_name,CASE WHEN r.status IN ('PAID','USED') THEN q.qr_token ELSE NULL END qr_token,q.status qr_status FROM reservations r JOIN performances p ON p.id=r.performance_id LEFT JOIN qr_tickets q ON q.reservation_id=r.id WHERE r.id=? AND (r.user_id=? OR ? IN ('MANAGER','SUPER_ADMIN'))`,[parts[3],u.id,u.role]);if(r){[r]=await attachReservationAnswers([r]);if(r.qr_token)r.qr_verify_url=qrVerifyUrl(r.qr_token);r.poster_url=publicImageUrl(r.poster_url,'/api/performance-poster',r.performance_id);}return r?json(res,200,r):json(res,404,{error:'예매를 찾을 수 없습니다.'});}
  ...
  if(req.method==='GET'&&url.pathname==='/api/admin/reservations'){const u=await requireRole(req,res,['MANAGER','SUPER_ADMIN']);if(!u)return;const rows=await all(`SELECT r.*,p.title,r.depositor_name user_name FROM reservations r JOIN performances p ON p.id=r.performance_id JOIN users u ON u.id=r.user_id WHERE (?='SUPER_ADMIN' OR p.manager_id=?) ORDER BY r.created_at DESC`,[u.role,u.id]);return json(res,200,await attachReservationAnswers(rows));}
  ...
  if(reservationCsvMatch){
    const u=await requireRole(req,res);if(!u)return;
    const performance=await get('SELECT * FROM performances WHERE id=?',[reservationCsvMatch[1]]);if(!canManagePerformance(u,performance))return json(res,404,{error:'공연을 찾을 수 없습니다.'});
    const rows=await attachReservationAnswers(await all(`SELECT r.id,r.reservation_no,p.title,r.depositor_name,u.email,r.phone,r.total_amount,r.status,r.created_at,CASE WHEN r.status IN ('PAID','USED') THEN q.qr_token ELSE '' END qr_token FROM reservations r JOIN performances p ON p.id=r.performance_id JOIN users u ON u.id=r.user_id LEFT JOIN qr_tickets q ON q.reservation_id=r.id WHERE r.performance_id=? ORDER BY r.created_at DESC`,[performance.id]));
    const questionLabels=[...new Set(rows.flatMap(row=>row.answers.map(answer=>answer.question_text)))];
    const header=['예매번호','공연명','예매자','이메일','연락처','결제금액','상태','예매일','QR확인URL',...questionLabels];
    const cell=value=>`"${String(value??'').replaceAll('"','""')}"`;
    const csv='\uFEFF'+[header.map(cell).join(','),...rows.map(row=>[row.reservation_no,row.title,row.depositor_name,row.email,row.phone,row.total_amount,row.status,row.created_at,row.qr_token?qrVerifyUrl(row.qr_token):'',...questionLabels.map(label=>row.answers.find(answer=>answer.question_text===label)?.answer_text||'')].map(cell).join(','))].join('\r\n');
