...
async function initDb() {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS performances (id SERIAL PRIMARY KEY, manager_id INTEGER REFERENCES users(id), title TEXT NOT NULL, genre TEXT NOT NULL, artists TEXT NOT NULL, description TEXT NOT NULL, poster_url TEXT NOT NULL, venue_name TEXT NOT NULL, venue_description TEXT DEFAULT '', address TEXT NOT NULL, start_at TEXT NOT NULL, booking_start_at TEXT DEFAULT (CURRENT_TIMESTAMP::TEXT), booking_close_at TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'OPEN', host_avatar_url TEXT DEFAULT '/assets/host-avatar.svg', artist_avatar_url TEXT DEFAULT '/assets/artist-avatar.svg', deposit_notice TEXT DEFAULT '신청 후 24시간 이내 입금', refund_policy TEXT DEFAULT '공연 취소 및 환불 규정을 확인해 주세요.', max_tickets_per_order INTEGER DEFAULT 4, created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP);
    ...
    CREATE TABLE IF NOT EXISTS performance_questions (id SERIAL PRIMARY KEY, performance_id INTEGER NOT NULL REFERENCES performances(id) ON DELETE CASCADE, question_text TEXT NOT NULL, description_text TEXT DEFAULT '', question_type TEXT DEFAULT 'select', is_required INTEGER DEFAULT 1, sort_order INTEGER DEFAULT 0);
    CREATE TABLE IF NOT EXISTS performance_question_options (id SERIAL PRIMARY KEY, question_id INTEGER NOT NULL REFERENCES performance_questions(id) ON DELETE CASCADE, option_text TEXT NOT NULL, description_text TEXT DEFAULT '', sort_order INTEGER DEFAULT 0);
...
function normalizePerformance(input) {
  ...
  const questions = (Array.isArray(input.questions) ? input.questions : []).map((question, index) => {
    ...
    return { question_text: questionText, description_text:String(question.description_text || '').trim(), question_type: questionType, is_required: question.is_required !== false && String(question.is_required) !== '0', options: questionType.includes('text') ? [] : uniqueOptions };
  });
...
async function performanceQuestions(performanceId, client = pool) {
  const rows = await all(`SELECT q.id,q.question_text,q.description_text question_description_text,q.question_type,q.is_required,q.sort_order,o.id option_id,o.option_text,o.description_text,o.sort_order option_sort_order
  ...
  for (const row of rows) {
    if (!grouped.has(row.id)) grouped.set(row.id, {id:row.id,question_text:row.question_text,description_text:row.question_description_text || '',question_type:row.question_type || 'select',is_required:Number(row.is_required)!==0,sort_order:Number(row.sort_order),options:[]});
    if (row.option_id) grouped.get(row.id).options.push({id:row.option_id,option_text:row.option_text,description_text:row.description_text || '',sort_order:Number(row.option_sort_order)});
  }
...
async function replacePerformanceQuestions(performanceId, questions, client = pool) {
  ...
  for (const [questionIndex, question] of questions.entries()) {
    const created = await get('INSERT INTO performance_questions(performance_id,question_text,description_text,question_type,is_required,sort_order) VALUES(?,?,?,?,?,?) RETURNING id', [performanceId, question.question_text, question.description_text, question.question_type, question.is_required ? 1 : 0, questionIndex + 1], client);
    for (const [optionIndex, option] of question.options.entries()) await run('INSERT INTO performance_question_options(question_id,option_text,description_text,sort_order) VALUES(?,?,?,?)', [created.id, option.option_text, option.description_text, optionIndex + 1], client);
  }
}
